import { useState } from "react";
import { NavLink } from "react-router-dom"; // for moving between pages when navigating the sidebar
import { 
  FaBars, FaTachometerAlt, FaUserFriends, FaCar, 
  FaExchangeAlt, FaExclamationTriangle, FaStar, FaFileAlt 
} from "react-icons/fa";
import "./adminSidebar.css"; 

const AdminSidebar = () => {
  const [collapsed, setCollapsed] = useState(false);

  const menuItems = [
    { name: "Dashboard", icon: <FaTachometerAlt />, path: "/admin/dashboard" },
    { name: "Passengers", icon: <FaUserFriends />, path: "/admin/passengers" },
    { name: "Car Owners", icon: <FaCar />, path: "/admin/car-owners" },
    { name: "Rental Transactions", icon: <FaExchangeAlt />, path: "/admin/rental-transactions" },
    { name: "Car Issues", icon: <FaExclamationTriangle />, path: "/admin/car-issues" },
    { name: "Review", icon: <FaStar />, path: "/admin/review" },
    { name: "Report", icon: <FaFileAlt />, path: "/admin/report" },
  ];

  return (
    <div className={`admin-sidebar ${collapsed ? "collapsed" : ""}`}>
      
      {/* Toggle Button */}
      <div className="admin-sidebar-toggle" onClick={() => setCollapsed(!collapsed)}>
  <FaBars className="toggle-icon" />
  {!collapsed && (
    <div className="admin-sidebar-title">
      CarCalling
    </div>
  )}
</div>

      <nav className="admin-sidebar-menu">
        {menuItems.map((item, index) => (
          <NavLink 
            key={index} 
            to={item.path} 
            className="admin-sidebar-item"
            activeclassname="active"
          >
            <span className="admin-sidebar-icon">{item.icon}</span>
            {!collapsed && <span className="admin-sidebar-text">{item.name}</span>}
          </NavLink>
        ))}
      </nav>
    </div>
  );
};

export default AdminSidebar;